...
$img = $_REQUEST['id'];
$img = $img;
$gallery = get_children( 'posts_per_page=-1&post_type=attachment&post_mime_type=image&post_parent=' . $img);
$attr = array(
    'class' => "attachment-$size wp-post-image",);
foreach( $gallery as $image ) {
    if ( has_post_thumbnail() ) { // check if the post has a Post Thumbnail assigned to it.
      $imgLink = the_post_thumbnail();
    }
    else{
          $imgLink = wp_get_attachment_url($image->ID);
    } 
} 
echo '<div id="bg"><a href="#" class="nextImageBtn" title="next"></a><a href="#" class="prevImageBtn" title="previous"></a><img src="' . $imgLink . '" width="1680" height="1050" alt="Test" title="Test" id="bgimg" /></div>';
?>
...
<?php
// $_REQUEST catches $_GET and $_POST
$id = $_REQUEST['id'];
$id = $id;
$gallery = get_children( 'posts_per_page=-1&post_type=attachment&post_mime_type=image&post_parent=' . $id);
$attr = array(
    'class' => "attachment-$size wp-post-image",
);
foreach( $gallery as $image ) {
  ...
  echo'<div class="content">';
     ...
     // echo '<div class="content">' . wp_get_attachment_image($image->ID, 'thumbnail', false, $attr) . '</div>';
     echo '<a href="../'.$link.'"><img src="' . $link . '"width="200px" height="130px" title="Testt" alt="test" class="thumb"/></a>';
     echo '</a>';
     echo $link;
     echo'</div>';
   }
